"""
LIBERO-10 evaluation with LLM orchestration + Scene-Aware MambaVLA.

GPT-4o decomposes each multi-step task into LIBERO-90 primitives.
MambaVLAScene (ResNet obs + V-JEPA z_scene conditioning) executes each
primitive sequentially with home-pose return.

Usage:
    python -m libero_bench.eval_libero10_llm_scene \
        --checkpoint ./checkpoints/libero_10_subset_scene/final_model.pth \
        --openai_api_key sk-... \
        --num_rollouts 3
"""
...
def run_eval(
    ...
    log.info(f"Loading MambaVLAScene from {checkpoint_path}")
    model = load_model(
        checkpoint_path,
        device=device,
        vjepa_model_name=vjepa_model_name,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--vjepa_model_name", type=str,
                        default="facebook/vjepa2-vitg-fpc64-256")
    parser.add_argument("--vjepa_n_finetune_layers", type=int, default=4)
    parser.add_argument("--vjepa_n_frames", type=int, default=4)
    ...
    run_eval(
        ...
        vjepa_n_finetune_layers=args.vjepa_n_finetune_layers,
        vjepa_n_frames=args.vjepa_n_frames,
