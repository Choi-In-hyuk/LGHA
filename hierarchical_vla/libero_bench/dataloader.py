...
class LiberoDataset():
    def __init__(
        ...
        # Try to find language_embeddings in multiple locations
        # First try in test folder, then in MambaVLA folder
        ...
        if not os.path.exists(task_emb_dir):
            ...
            if os.path.exists(mambavla_emb_dir):
                ...
                raise FileNotFoundError(
                    f"Language embeddings directory not found. Tried:\n"
                    f"  - {task_emb_dir}\n"
                    f"  - {mambavla_emb_dir}"
                )
        ...
        if not os.path.exists(task_emb_path):
            raise FileNotFoundError(
                f"Language embedding file not found: {task_emb_path}\n"
                f"Available files in {task_emb_dir}: {os.listdir(task_emb_dir) if os.path.exists(task_emb_dir) else 'N/A'}"
            )
    ...
    def camera_names(self):
        """
        Returns the list of camera names used in this dataset.
        These are the base names (without '_image' suffix) of the image observation keys.
        """
